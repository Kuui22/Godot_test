...
#that func (set_player_inventory_data) calls set_inventory_data func that calls populate_item_grid
#how that works is that pop_i_g sets the inventory data, (which is an array of slot datas) with slot datas by looping through
...
#item data has subclasses like consumable and equip which will have some extra values/features
#also for new types/subclasses there can be extra inventories, like the equip
#these extra inventories will have super functions
#these super functions modify normal functions to change base inventory interactions of these item_datas
